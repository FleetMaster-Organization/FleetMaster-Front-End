...
export const useAlertsStore = defineStore('alerts', () => {
    ...
    async function loadAlerts() {
        ...
        try {
            let res = await api.get<any[]>('/alerts')
            // Si la base de datos de alertas está vacía, pre-generamos activamente las alertas en el backend
            if ((!res.data || res.data.length === 0) && !hasAttemptedAutoGeneration) {
                hasAttemptedAutoGeneration = true
                try {
                    await api.post('/alerts/generate')
                    res = await api.get<any[]>('/alerts')
                } catch (genError) {
                    console.warn('Auto-generation of alerts skipped or failed:', genError)
                }
            }
