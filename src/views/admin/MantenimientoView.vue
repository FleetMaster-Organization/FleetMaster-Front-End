...
function matchSearch(r: MaintenanceRecord) {
...
const columnsTaller = [
    ...
    { key: 'tecnico',   label: 'Técnico', width: '140px' },
    { key: 'costo',     label: 'Costo', width: '130px', align: 'right' as const },
...
const columnsHistorial = [
    ...
    { key: 'kmIngreso',  label: 'Km ingreso', width: '100px', align: 'right' as const },
    { key: 'kmSalida',   label: 'Km salida',  width: '100px', align: 'right' as const },
    { key: 'costo',      label: 'Costo', width: '130px', align: 'right' as const },
    { key: 'proximoMant',label: 'Próx. mantenimiento', width: '150px' },
...
// ── Vehículos elegibles para mantenimiento ────────────────────
...
<template>
    <div class="p-6 space-y-6">
        ...
        <button
            v-for="tab in [{ key: 'taller', label: 'En taller', count: maintenanceStore.abiertos.length },
            ...
            @click="activeTab = tab.key as 'taller' | 'historial'"
        >
            {{ tab.label }}
        ...
        <template #cell-fechaIngreso="{ row }">
            <span class="text-xs text-slate-600">{{ fmtDate((row as MaintenanceRecord).fechaIngreso) }}</span>
        </template>

        <template #cell-kmIngreso="{ row }">
            <span class="font-mono text-xs text-slate-600">{{ fmtKm((row as MaintenanceRecord).kilometrajeIngreso) }}</span>
        </template>
        ...
        <template #cell-vehiculo="{ row }">
            ...
            <p class="text-xs text-slate-400">{{ (row as MaintenanceRecord).vehiculoMarca }} {{ (row as MaintenanceRecord).vehiculoModelo }}</p>
            </div>
        </template>
        ...
        <template #cell-fechaSalida="{ row }">
            <span class="text-xs text-slate-600">{{ fmtDate((row as MaintenanceRecord).fechaSalida) }}</span>
        </template>
        ...
        <template #cell-kmSalida="{ row }">
            <span class="font-mono text-xs text-slate-600">{{ fmtKm((row as MaintenanceRecord).kilometrajeSalida) }}</span>
        </template>
        ...
        <template #cell-proximoMant="{ row }">
            <span v-if="(row as MaintenanceRecord).proximoMantenimiento"
        ...
        <!-- ── Modal Abrir Mantenimiento ─────────────────────── -->
        <BaseModal :show="showFormModal" title="Abrir registro de mantenimiento" size="lg" @close="showFormModal = false">
        <div class="space-y-4">
            <!-- Vehículo -->
            <div>
            ...
            <div>
                ...
                <input v-model="form.fechaIngreso" type="date" :max="todayStr()"
                class="w-full px-3 py-2 text-sm border border-slate-200 rounded-lg bg-white
                        focus:outline-none focus:ring-2 focus:ring-blue-500/30 focus:border-blue-400 transition-all"
                />
            </div>
            ...
            <!-- Costo -->
            <div>
                <label class="block text-xs font-semibold text-slate-600 mb-1.5">Costo estimado (COP)</label>
            ...
            <div>
                ...
                <input v-model="form.tecnico" type="text"
                class="w-full px-3 py-2 text-sm border border-slate-200 rounded-lg bg-white
                        focus:outline-none focus:ring-2 focus:ring-blue-500/30 focus:border-blue-400 transition-all"
            ...
            <strong>Mantenimiento</strong> y quedará bloqueado para asignaciones.
            </div>
        ...
        <!-- ── Modal Cerrar Mantenimiento ──────────────────────── -->
        <BaseModal :show="showCloseModal" title="Cerrar registro de mantenimiento" size="md" @close="showCloseModal = false">
        <div class="space-y-4">
            <div v-if="closingTarget" class="p-3 bg-slate-50 border border-slate-200 rounded-lg text-sm">
            <p class="font-semibold text-slate-700">
                {{ closingTarget.vehiculoPlaca }} — {{ closingTarget.vehiculoMarca }} {{ closingTarget.vehiculoModelo }}
            </p>
            ...
            <div>
                ...
                <input v-model="closeForm.fechaSalida" type="date" :max="todayStr()"
                class="w-full px-3 py-2 text-sm border border-slate-200 rounded-lg bg-white
                        focus:outline-none focus:ring-2 focus:ring-blue-500/30 focus:border-blue-400 transition-all"
                />
            </div>
            <div>
                <label class="block text-xs font-semibold text-slate-600 mb-1.5">
                Kilometraje de salida <span class="text-red-500">*</span>
                </label>
                <input v-model.number="closeForm.kilometrajeSalida" type="number"
            ...
            <div>
            <label class="block text-xs font-semibold text-slate-600 mb-1.5">Comentarios de cierre</label>
            <textarea v-model="closeForm.comentariosCierre" rows="2"
                class="w-full px-3 py-2 text-sm border border-slate-200 rounded-lg bg-white resize-none
                    focus:outline-none focus:ring-2 focus:ring-blue-500/30 focus:border-blue-400 transition-all"
            ...
            <!-- REQ-34/35: próximo mantenimiento (solo informativo) -->
            <div>
            <label class="block text-xs font-semibold text-slate-600 mb-1.5">
                Próximo mantenimiento programado
                <span class="text-slate-400 font-normal">(opcional — solo informativo)</span>
            </label>
            <input v-model="closeForm.proximoMantenimiento" type="date" :min="todayStr()"
                class="w-full px-3 py-2 text-sm border border-slate-200 rounded-lg bg-white
                    focus:outline-none focus:ring-2 focus:ring-blue-500/30 focus:border-blue-400 transition-all"
            />
